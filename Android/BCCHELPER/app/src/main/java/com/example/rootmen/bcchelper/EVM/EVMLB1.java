...
public class EVMLB1 extends AppCompatActivity {
    ...
    void toMk() {
        ...
        try {
            ...
            if (Qtext != null)
                Qtext.setText("Ошибка при чтении данных проверьте правильности и последовательность МК");
        }
        MkText = new String("");
        String My[] = MK.split(" ");
    ...
    boolean NextMK(String MK) {
        ...
        if (MKi == 1) {
            ...
            if (PrD == null) {
                MkText = MkText + "\n PrD - Шина данных еще не опредлена";
                return false;
            }
            PrB = PrD;
            ...
            MkText = MkText + "\n       PrB=(Обнулился)=" + 0;
            PrB = new Integer(0);
            MkText = MkText + "\n       PrC=(Прием с АЛУ)=ИНВЕРСИЯ A=" + toHex(toUnver(PrA));
            PrC = toUnver(PrA);
            MkText = MkText + "\n       PrD=Шине входа=" + toHex(B);
            PrD = B;
        } else if (MKi == 8) {
            ...
            MkText = MkText + "\n       PrC=(AЛУ)=Av(инверсная B)=" + toHex(PrD | toUnver(PrB));
            PrC = PrD | toUnver(PrB);
            MkText = MkText + "\n       PrD=(Хранение, выдача на Шн.д)=" + toHex(PrD);
        } else if (MKi == 18) {
